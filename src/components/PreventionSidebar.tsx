...
// GAIA Prevent only lists hazards that can actually be anticipated ahead of time,
...
// are deliberately excluded — they are handled in GAIA Route, which is a response
 ...
 * radius when handing a hazard off to GAIA Route. Falls back to a sensible default
...
function buildMarkdownReport(
  ...
  lines.push(`# GAIA Prevent — Hazard Analysis Report`);
  lines.push("");
  ...
  lines.push(`**Source:** GAIA — OpenStreetMap geometry, Open-Meteo climate & elevation, GDACS/USGS live feeds`);
  lines.push("");
  ...
  lines.push(`---`);
  lines.push("");
  lines.push(`_Data: OpenStreetMap (ODbL) · Open-Meteo · GDACS · USGS · NASA FIRMS · OpenAQ. Generated by Envis GAIA Prevent._`);
...
export default function PreventionSidebar({
  ...
  const handlePublishAlert = async (hazardType: string) => {
    ...
    const message = forecastEntry
      ...
      : analysis.checklist?.[0] || `${hazardType} exposure identified for this location — see GAIA Prevent for details.`;
            ...
            <LayoutHeader hasDivider padding={6}>
              <VStack gap={0.5}>
                <HStack gap={2} vAlign="center">
                  ...
                  <Heading level={2}>GAIA Prevent</Heading>
                </HStack>
            ...
            <LayoutContent padding={6} isScrollable={false}>
              <VStack gap={4}>
                ...
                <VStack gap={2}>
                  ...
                  <Text type="supporting" size="3xs" style={{ color: "var(--color-text-secondary)" }}>
                    Unpredictable events (spills, radiation, earthquakes) are handled in GAIA Route for response, not here.
                  </Text>
